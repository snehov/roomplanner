...
class BodyView extends Component {
    ...
    render() {
        ...
        let tableLastRowAddedHeight = schedulerData.getTableLastRowAddedHeight();

        let displayRenderData = renderData.filter(o => o.render);
        let tableRows = displayRenderData.map((item, rowIndex) => {
            ...
            var currentRowHeight = rowIndex === displayRenderData.length - 1 ? { height:item.rowHeight + tableLastRowAddedHeight } : { height: item.rowHeight };
            return (
                <tr key={item.slotId} style={currentRowHeight}>
                    {rowCells}
                </tr>
            );
        });
